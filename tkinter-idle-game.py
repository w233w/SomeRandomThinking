...
class UI:
    ...
    def main_page(self):
        ...
        # 扭曲
        self.twist_rate_tag = tk.Label(
            self.Upper, text="扭曲度:", bd=0.5, relief="ridge"
        )
    ...
    def shop_page(self):
        ...
        self.gold = tk.StringVar(value=self.values.gold)
        ...
        # 在按钮可用性层面解决购买金钱不够的情况
        def shopping_item(item: str, val: int):
            if self.values.gold >= val:
                self.values.gold -= val
            else:
                self.update_status(" Not enough money.")
            match item:
                case "1号商品":
                    self.values.sans += 3
                case "2号商品":
                    self.values.insight += 1

        for i, (k, v) in enumerate(self.shopping_items.items()):
            tk.Label(self.shopping_frame, text=k, anchor="w").place(
                relx=0, rely=i / 7, relheight=1 / 7, relwidth=0.7
            )
            tk.Button(
                self.shopping_frame,
                text=f"${v}",
                command=lambda k=k, v=v: shopping_item(k, v),
            ).place(relx=0.7, rely=i / 7, relheight=1 / 7, relwidth=0.3)
    ...
    def update_shop(self):
        ...
        self.root.after(200, self.update_shop)
